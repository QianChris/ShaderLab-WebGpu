...
export function init(ctx) {
    // Derive initial spherical coords from camera position (target assumed at origin)
...
export function update(_ctx) {
    ...
    // Quaternion: rotate (0,0,-1) to forward direction
    // q = q_yaw(+az) * q_pitch(-el) = [-ca*se, ce*sa, sa*se, ca*ce]
    // where sa=sin(az/2) ca=cos(az/2) se=sin(el/2) ce=cos(el/2)
