// OrbitCameraController — LEFT half of the screen (multi-view).
// Left drag: orbit (azimuth + elevation)
// Right drag: pan target
// Wheel: zoom distance
//
// Multi-view: this camera only responds to pointer input that lands in the
// left half of the canvas (NDC x < 0). The right half drives the other camera.
...
export function init(ctx) {
    ...
    ctx.on('mousedown', (e) => {
        if (e.x >= 0) return; // only the left half
        dragButton = e.button;
        prevX = e.x;
        prevY = e.y;
    });
    ...
    ctx.on('wheel', (e) => {
        if (e.x >= 0) return; // only zoom this camera when the wheel is in the left half
